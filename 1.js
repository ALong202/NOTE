...
function buildTree(dir) {
  ...
  for (const item of items) {
    ...
    if (item.isDirectory()) {
      result.push({ type: "dir", name: item.name, children: buildTree(fullPath) });
...
// 🔹 Render Markdown dạng note đẹp, vẫn có <details>
function renderTree(nodes, depth = 0) {
  const lines = [];
  const indent = "  ".repeat(depth);

  // icon cho từng cấp độ “notebook style”
  const sectionIcons = ["📘", "📗", "📙", "📒", "📔"];
  const fileIcon = "🪶";
...
console.log("✅ Đã tạo/cập nhật index.md với giao diện note style đẹp hơn.");
